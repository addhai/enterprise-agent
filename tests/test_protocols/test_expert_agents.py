...
class TestExpertDelegationTools:
    ...
    def test_perf_delegation_tool_description(self):
        ...
        assert (
            "性能" in perf_tool.description
            or "performance" in perf_tool.description.lower()
        )
    ...
    def test_security_delegation_tool_description(self):
        ...
        assert (
            "安全" in sec_tool.description or "security" in sec_tool.description.lower()
        )
...
class TestDelegateToSecurityExpert:
    ...
    def test_delegate_to_security_expert_local_fallback_login(self):
        ...
        result = asyncio.run(
            delegate_to_security_expert("suspicious login from unknown IP")
        )
        assert result is not None
        assert "可疑登录" in result
...
class TestExpertAgentConfig:
    ...
    def test_expert_timeout_config(self):
        ...
        # 2026-09 对齐：默认值已从 30 提到 120（src/config.py，cs 委托 LLM 较慢），
        # 测试跟随代码单一真相，勿再回写 30。
        assert settings.a2a_expert_timeout == 120
