...
def test_run_echoes_history_into_messages(make_agent):
    ...
    result = agent.run(
        "第二步做什么？", chat_history=[("第一步：登录", "请打开登录页")]
    )
...
def test_run_safe_on_llm_failure(make_agent):
    ...
    # 关键契约：内部错误细节绝不出现在对用户的最终回复里。
    # 注意：不能裸查 "503" 子串——兜底消息里的随机会话 ID（hex）可能恰好
    # 包含 "503"（CI 真实翻车案例：会话 ID 3b3503b55001 导致误报），
    # 改为查注入的错误原文 "upstream"，语义等价且不会被随机 ID 误伤。
